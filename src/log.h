...
namespace {
constexpr auto ltoi(Level level) { return std::to_underlying(level); }
...
constexpr void warn(fmt::format_string<T...> format_string, T&&... args) {
    if (ltoi(level()) < ltoi(Level::WARN))
        return;
    fmt::print(stderr, fmt::emphasis::bold | fmt::fg(fmt::color::orange),
               "[warn] ");
...
constexpr void error(fmt::format_string<T...> format_string, T&&... args) {
    ...
    Error err(Error::UNKNOWN_ERROR,
              fmt::format(format_string, std::forward<T>(args)...));
    error(err);
}
