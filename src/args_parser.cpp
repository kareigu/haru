...
constexpr const char* EPILOG =
    HARU_PRG_NAME " v" HARU_VERSION " - " RELEASE_TYPE;
...
void init() {
    ...
    s_create_cmd.add_argument("-d", "--use-defaults")
        .flag()
        .help("Use defaults for all possible fields");
    s_create_cmd.add_argument("-f", "--force").flag();

    s_init_cmd.add_description("Initialise a project in the current directory");
    s_init_cmd.add_argument("-d", "--use-defaults")
        .flag()
        .help("Use defaults for all fields");
    s_init_cmd.add_argument("-f", "--force").flag();

    s_config_cmd.add_description("Access configuration options");
    s_config_cmd.add_argument("key").nargs(0, 1).help(
        "Key of configuration value to access");
    s_config_cmd.add_argument("value").remaining().nargs(0, 1).help(
        "Value to set");
    ...
    s_cmake_init_cmd.add_description(
        "Initialise CMake using a available config");
    s_cmake_init_cmd.add_argument("-g", "--global")
        .flag()
...
std::expected<Command, Error> parse(int argc, char** argv) {
    ...
    if (s_prg.is_subcommand_used(s_config_cmd)) {
        ...
        if (args.empty() && !(flags & Command::Flags::PATH))
            return std::unexpected(
                Error(Error::NO_INPUT, "No key/value provided"));
    ...
    if (s_prg.is_subcommand_used(s_cmake_cmd)) {
        if (s_cmake_cmd.is_subcommand_used(s_cmake_init_cmd)) {
            ...
            return Command{
                .type = Command::CMAKE_INIT, .flags = flags, .args = {}};
        }

        return std::unexpected(
            Error(Error::INPUT_ERROR, "Subcommand needs to be provided"));
    }

    return std::unexpected(
        Error(Error::UNKNOWN_ERROR, "Unhandled command provided"));
}
...
std::string usage_string() { return help_string(); }
} // namespace arg_parse
} // namespace haru
