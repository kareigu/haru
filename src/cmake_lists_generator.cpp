...
namespace haru {
CMakeListsGenerator::CMakeListsGenerator(const ProjectInfo& project_info)
...
constexpr const char* LINK_DEPENDENCIES_FN =
    "link_project_dependencies(${PROJECT_NAME})";
constexpr const char* ADD_TIDY_FN =
...
void CMakeListsGenerator::write_templated_string(
    std::stringstream& output, const std::string_view placeholder_name) const {
    ...
    if (placeholder_name == "CXX_STANDARD") {
        if (m_project_info.languages & Language::CPP)
            output << "CXX_STANDARD "
                   << m_project_info.standard[ProjectInfo::CPP_INDEX];
        return;
    }
    if (placeholder_name == "C_STANDARD") {
        if (m_project_info.languages & Language::C)
            output << "C_STANDARD "
                   << m_project_info.standard[ProjectInfo::C_INDEX];
        return;
    }
...
CMakeListsGenerator::generate_cmakelists() const {
    std::stringstream output;

    std::string_view cmake_template(
        reinterpret_cast<char*>(&bake_in_cmakelists),
        reinterpret_cast<char*>(&bake_in_cmakelists[bake_in_cmakelists_len]));
    ...
    if (output.fail() || output.bad())
        return std::unexpected(
            Error(Error::GENERATE_ERROR,
                  "Error writing generated CMakesLists.txt to buffer"));

    return output.str();
}

std::expected<std::string, Error>
CMakeListsGenerator::generate_dependencies() const {
    std::stringstream output;

    std::string_view cmake_template(
        reinterpret_cast<char*>(&bake_in_cmake_dependencies),
        reinterpret_cast<char*>(
            &bake_in_cmake_dependencies[bake_in_cmake_dependencies_len]));
    for (size_t i = 0; i < cmake_template.size(); i++) {
        auto template_opt = gather_templated_string(cmake_template, i);
        if (template_opt.has_value())
            write_templated_string(output, template_opt.value());
    ...
    if (output.fail() || output.bad())
        return std::unexpected(
            Error(Error::GENERATE_ERROR,
                  "Error writing generated compile_flags.cmake to buffer"));

    return output.str();
}

std::expected<std::string, Error>
CMakeListsGenerator::generate_compile_flags() const {
    std::stringstream output;
...
std::expected<std::string, Error> CMakeListsGenerator::generate_tidy() const {
    std::stringstream output;
    std::string_view cmake_template(
        reinterpret_cast<char*>(&bake_in_cmake_tidy),
    ...
    if (output.fail() || output.bad())
        return std::unexpected(
            Error(Error::GENERATE_ERROR,
                  "Error writing generated tidy.cmake to buffer"));

    return output.str();
}

std::expected<std::vector<CMakeListsGenerator::CMakeFile>, Error>
CMakeListsGenerator::generate() {
    std::vector<CMakeFile> files;
    files.reserve(4);

    std::string cmakelists_contents = TRY(generate_cmakelists());
    files.emplace_back(CMakeFile{.filepath = "CMakeLists.txt",
                                 .contents = std::move(cmakelists_contents)});

    if (should_generate_dependencies()) {
        std::string dependencies_contents = TRY(generate_dependencies());
        files.emplace_back(
            CMakeFile{.filepath = "cmake/dependencies.cmake",
                      .contents = std::move(dependencies_contents)});
    }
