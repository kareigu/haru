...
int main(int argc, char** argv) {
    ...
    if (argc <= 1) {
        haru::log::error(
            haru::Error(haru::Error::INPUT_ERROR, "No arguments provided"));
    ...
    if (ran_command.type == haru::Command::CREATE ||
        ran_command.type == haru::Command::INIT) {
        ...
        auto project_info = MUST(haru::ProjectInfo::parse_from_input(
            ran_command.flags, default_name));

        haru::CMakeListsGenerator cmake_generator(project_info);
        auto cmake_files = MUST(cmake_generator.generate());

        std::filesystem::path workpath =
            MUST(haru::file_ops::create_work_directory(
                init, project_info.name,
                ran_command.flags & haru::Command::Flags::FORCE));
        MUST(haru::file_ops::write_cmake_files(workpath, cmake_files));
        MUST(haru::file_ops::write_entry_point(
            workpath, project_info.entry_point, project_info.languages));
        MUST(haru::file_ops::write_default_files(workpath,
                                                 project_info.default_files));
        ...
        WARN(haru::file_ops::format_generated_files(
            workpath, format_generated_files_args));

        MUST(haru::handle_post_ops(workpath, project_info.languages,
                                   ran_command.flags &
                                       haru::Command::Flags::USE_DEFAULTS));
    } else if (ran_command.type == haru::Command::CONFIG) {
        MUST(haru::Config::handle_config_command(ran_command.args,
                                                 ran_command.flags));
