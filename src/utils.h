 ...
 * @param default_value - default value if nothing is entered. If empty, Error
 * will be returned
 * @param new_line - whether to print a new line after prompt text
 * @return T on success, on error Error with more information
 */
template <typename T>
std::expected<T, Error> prompt(const char* text,
                               std::optional<T> default_value = {},
                               bool new_line = false) {
    auto default_value_formatted =
        default_value.has_value() ? fmt::format(" ({})", default_value.value())
                                  : "";
    ...
    if (!default_value.has_value() && value_input.empty())
        return std::unexpected(Error(
            Error::Type::NO_INPUT, fmt::format("{} needs to be given", text)));
...
prompt_list(const char* text, std::vector<T> valid_values,
            std::optional<std::vector<T>> default_values = {},
            bool new_line = false) {
...
empty_defaults:
    ...
    if (valid_values.empty())
        return std::unexpected(
            Error(Error::UNKNOWN_ERROR,
                  "Implementation error, missing valid values"));
    ...
    fmt::print("{:s} {:s}{:s}: ", text, valid_values_formatted,
    ...
    for (size_t i = 0; i < value_input.size(); i++) {
        ...
        inputs.push_back(std::istringstream(std::string(
            value_input.data() + prev_start, value_input.data() + i)));
        prev_start = i + 1;
    }
    inputs.push_back(std::istringstream(
        std::string(value_input.data() + prev_start,
                    value_input.data() + value_input.size())));
    ...
    for (size_t i = 0; i < inputs.size(); i++) {
        ...
        if (input.bad() || input.fail())
            return std::unexpected(Error(
                Error::INPUT_ERROR,
                fmt::format("Invalid value provided: {:s}, valid values: {:s}",
                            input.str(), valid_values_formatted)));
...
prompt_yes_no(std::string_view text, bool default_value, bool new_line = false);

std::expected<void, Error>
check_command_exists(const std::string_view command,
                     const std::string_view flags = "--version");
