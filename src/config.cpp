...
const std::filesystem::path Config::get_global_config_path() {
    ...
    auto path = std::filesystem::absolute(
        fmt::format("{:s}\\.config\\{:s}", home, CONFIG_FILE_NAME));
    return path;
}
#else
const std::filesystem::path Config::get_global_config_path() {
    ...
    auto path = std::filesystem::absolute(
        fmt::format("{:s}/{:s}", home, CONFIG_FILE_NAME));
    return path;
}
#endif
...
std::optional<const std::filesystem::path> Config::get_local_config_path() {
    ...
    while (current_path != "/") {
        auto possible_config =
            fmt::format("{}/{:s}", current_path, CONFIG_FILE_NAME);
...
std::expected<Config::Config_t, Error>
Config::get_config(const std::filesystem::path& path) {
    ...
    while (true) {
        ...
        if (line.contains('=')) {
            ...
            std::string_view local_key(line.begin(),
                                       line.begin() + split_point);
        ...
        if (file.fail())
            return std::unexpected(
                Error(Error::IO_ERROR,
                      fmt::format(
                          "Failed while reading config file at line {:d}: {:s}",
                          current_line, line)));

        return std::unexpected(Error(
            Error::CONFIG_ERROR,
            fmt::format("Invalid configuration value at line {:d}: '{:s}'",
                        current_line, line)));
    }
...
std::expected<void, Error> Config::write_value(const std::string& key,
        ...
        if (!local)
            return std::unexpected(
                Error(Error::NOT_FOUND,
                      "Create a config file inside current project first"));
    ...
    if (write_file.fail())
        return std::unexpected(
            Error(Error::IO_ERROR, "Failed while writing config file"));
    write_file.close();
    return {};
}

std::expected<void, Error>
Config::handle_config_command(const std::vector<std::string>& args,
                              const Command::Flags_t flags) {
