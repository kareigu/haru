...
create_work_directory(bool init, const std::string& name, bool overwrite) {
    ...
    if (!init) {
        ...
        if (std::filesystem::exists(workpath)) {
            if (!overwrite)
                return std::unexpected(Error(
                    ...
                    fmt::format(
                        "{:s} already exists at {}. Use '--force' to overwrite",
                        name, workpath)));
            ...
            if (ec)
                return std::unexpected(
                    Error(Error::IO_ERROR,
                          fmt::format(
                              "Could not remove existing directory at {}, {:s}",
                              workpath, ec.message())));
        }
        ...
        if (!std::filesystem::create_directory(workpath)) {
            return std::unexpected(Error(
                Error::IO_ERROR,
                fmt::format("Could not create directory at {}", workpath)));
        }
    ...
    if (!std::filesystem::is_empty(workpath)) {
        bool proceed =
            overwrite ? true
                      : TRY(prompt_yes_no(
                            "Current directory is not empty, proceed?", false));
        if (!proceed)
            return std::unexpected(
                Error(Error::IO_ERROR, "Current directory is not empty"));
    }
...
write_cmake_files(const std::filesystem::path& workpath,
    ...
    for (const auto& file : files) {
        ...
        if (file.filepath.has_parent_path()) {
            ...
            if (!std::filesystem::exists(file_directory) &&
                ...
                return std::unexpected(
                    Error(Error::IO_ERROR,
                          fmt::format("Couldn't create directories for path {}",
                                      file_directory)));
        }
        filepath += "/";
        ...
        if (output.fail() || output.bad())
            return std::unexpected(
                Error(Error::WRITE_ERROR,
                      fmt::format("Failed writing {}", filepath)));
...
std::expected<void, Error>
write_entry_point(const std::filesystem::path& workpath,
                  const std::string& entry_point, Language_t languages) {
    ...
    if (!std::filesystem::exists(directory) &&
        ...
        return std::unexpected(
            Error(Error::IO_ERROR,
                  fmt::format("Failed creating directory {}", directory)));

    std::ofstream output(full_path);
    if (!output.is_open())
        return std::unexpected(Error(
            Error::IO_ERROR, "Could not open entry point file for writing"));
    ...
    if (output.fail() || output.bad())
        return std::unexpected(Error(
            Error::WRITE_ERROR, fmt::format("Failed writing {}", full_path)));
...
std::expected<void, Error>
write_default_files(const std::filesystem::path& workpath,
    ...
    if (default_files & DefaultFiles::GERSEMIRC) {
        ...
        if (output.bad() || output.fail())
            return std::unexpected(
                Error(Error::WRITE_ERROR, "Failed writing .gersemirc"));
        log::info("Wrote {}", filepath);
    }
    if (default_files & DefaultFiles::GITIGNORE) {
        ...
        if (output.bad() || output.fail())
            return std::unexpected(
                Error(Error::WRITE_ERROR, "Failed writing .gitignore"));
        log::info("Wrote {}", filepath);
    }
    return {};
}
...
std::expected<void, Error>
format_generated_files(const std::filesystem::path& workpath,
                       FormatGeneratedFilesArgs args) noexcept {
...
std::expected<void, Error>
format_cmake_files(const std::filesystem::path& workpath,
                   const std::vector<CMakeListsGenerator::CMakeFile>& files) {
    ...
    std::for_each(
        ...
        [&workpath, &failed](const CMakeListsGenerator::CMakeFile& file) {
            ...
            if (std::system(fmt::format("cd {} && {:s} -i {}", workpath,
                                        CMAKE_FORMATTER, filepath)
                                .c_str())) {
                failed = Error(Error::EXEC_ERROR,
                               fmt::format("Could not format {:s} using {}",
                                           filepath, CMAKE_FORMATTER));
                return;
            }
...
std::expected<void, Error>
format_source_files(const std::filesystem::path& workpath,
    ...
    if (std::system(fmt::format("cd {} && {:s} -i {:s}", workpath,
                                SOURCE_FORMATTER, source_path)
                        .c_str()))
        return std::unexpected(Error(
            Error::EXEC_ERROR, fmt::format("Could not format {:s} using {:s}",
                                           source_path, SOURCE_FORMATTER)));
