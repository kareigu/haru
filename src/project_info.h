...
constexpr DefaultFiles_t CLANG_TIDY = 2;
constexpr DefaultFiles_t GERSEMIRC = 4;
constexpr DefaultFiles_t GITIGNORE = 8;
constexpr DefaultFiles_t ALL =
    CLANG_FORMAT | CLANG_TIDY | GERSEMIRC | GITIGNORE;
...
struct ProjectInfo {
    ...
    static std::expected<ProjectInfo, Error>
    parse_from_input(Command::Flags_t flags,
...
//
template <> struct fmt::formatter<haru::Dependency> {
    template <typename ParseContext> constexpr auto parse(ParseContext& ctx) {
        return ctx.begin();
    }

    auto format(haru::Dependency dependency, format_context& ctx) const {
        const char* source = dependency.source == haru::Dependency::Source::GIT
                                 ? "git"
                                 : "local";
        fmt::format_to(ctx.out(), "{{\n");
        fmt::format_to(ctx.out(), " name = {:s}\n", dependency.name);
        const char* location_name =
            dependency.source == haru::Dependency::Source::GIT ? "url" : "path";
...
template <> struct fmt::formatter<haru::ProjectInfo> {
    template <typename ParseContext> constexpr auto parse(ParseContext& ctx) {
        return ctx.begin();
    }
